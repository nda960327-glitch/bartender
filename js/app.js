...
(function () {
  ...
  // 30석 가게 · 칵테일 구독만으로 운영하는 설계 (구독 매출 월 2,000만 원 목표 — docs/하우스패스_구독전용_운영안.html)
  // 요일 제한 없이 월 잔수 상한과 정원으로 자리를 지킵니다. 방문 횟수 대신 "한 번에 2잔"(본인 2잔 또는 동반 1잔)으로 객단가를 올려요.
  // 위스키는 상품이 아니라 회원 혜택(10% 할인)이에요.
  const PASS_PRESET_PLANS = [
    { name: "위클리", price: 59000, kind: "personal", days: "all", drinks_per_day: 1, monthly_cap: 8, team_size: 1, duration_days: 30, max_members: 42, sort: 1,
      note: "주 2회 퇴근 한 잔. 하루 1잔 · 월 8잔, 4번만 와도 본전. 회원은 위스키 10% 할인." },
    { name: "스탠다드", price: 89000, kind: "personal", days: "all", drinks_per_day: 1, monthly_cap: 12, team_size: 1, duration_days: 30, max_members: 60, sort: 2,
      note: "주 3회. 하루 1잔 · 월 12잔, 잔당 7,400원. 회원은 위스키 10% 할인." },
    { name: "더블", price: 129000, kind: "personal", days: "all", drinks_per_day: 2, monthly_cap: 16, team_size: 1, duration_days: 30, max_members: 40, sort: 3,
      note: "하루 2잔 · 월 16잔. 혼자 2잔도, 동료 1잔 사주기도 돼요. 회원은 위스키 10% 할인." },
    { name: "데일리", price: 149000, kind: "personal", days: "all", drinks_per_day: 1, monthly_cap: 24, team_size: 1, duration_days: 30, max_members: 10, sort: 4,
      note: "거의 매일 오는 분. 하루 1잔 · 월 24잔. 회원은 위스키 10% 할인." },
    { name: "시그니처", price: 179000, kind: "personal", days: "all", drinks_per_day: 2, monthly_cap: 20, team_size: 1, duration_days: 30, max_members: 15, sort: 5,
      note: "시그니처·프리미엄 칵테일(정가 18,000~)까지. 하루 2잔 · 월 20잔. 위스키 10% 할인." },
    { name: "팀 패스", price: 249000, kind: "team", days: "all", drinks_per_day: 1, monthly_cap: 30, team_size: 5, duration_days: 30, max_members: 10, sort: 6,
      note: "5명 등록 · 각자 하루 1잔 · 팀 합산 월 30잔. 1인 49,800원, 법인카드 OK." },
    { name: "원데이", price: 18000, kind: "oneday", days: "all", drinks_per_day: 2, monthly_cap: null, team_size: 1, duration_days: 1, max_members: null, sort: 7,
      note: "비회원 · 당일 2잔. 회원이 데려온 동료용." },
    { name: "스탠다드 3개월", price: 249000, kind: "personal", days: "all", drinks_per_day: 1, monthly_cap: 12, team_size: 1, duration_days: 90, max_members: 5, sort: 8,
  ...
  function renderPassPlansTab(area, a) {
    ...
    area.innerHTML = `
      ...
      <div class="card">
        ...
        <button class="host-chat-btn" id="pa-preset" style="margin-top:8px">🏁 운영안 상품 8종 채우기 <small style="font-weight:600;color:var(--text-sub)">(있는 건 건너뜀)</small></button>
      ...
      <p class="pass-note" style="margin:10px 20px 24px">"무제한"이라는 말은 쓰지 마세요 — 잔수와 상한으로 적습니다. 원가 3,000원 넘는 메뉴는 1잔 풀에 넣지 않는 게 원칙이에요. 정원은 하루 방문이 좌석 수의 2배를 넘지 않게 잡아요 (30석이면 회원 200명 안팎). 정원이 차면 "정원 마감"으로 보이고 자리가 나면 다시 열려요. 위스키 회원 할인 같은 혜택은 안내문과 상품 설명에 적어 주세요.</p>`;
    ...
    if (preset) preset.addEventListener("click", async () => {
      ...
      if (!todo.length) { toast("운영안 상품 8종이 이미 다 있어요."); return; }
