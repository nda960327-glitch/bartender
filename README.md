...
채용 화면은 있는데 지원할 "사람" 쪽 이력이 없었습니다. 경력·지역·시그니처·자격증·
가능 업무를 한 장으로 정리해두는 화면이에요.

이 기기에만 저장되고 서버로 올라가지 않습니다. 오른쪽 위 복사 버튼을 누르면
공고에 그대로 붙여넣을 수 있는 텍스트로 만들어줘요. 연락처 칸도 같은 이유로
"꼭 필요하면 적으라"고만 안내합니다.
...
**주소와 전화번호는 일부러 받지 않습니다.** 확인되지 않은 정보가 앱에서 사실처럼
퍼지면 그 가게에 폐가 되니까요. 동네·영업시간·시그니처·분위기까지만 받습니다.
...
- 기본 목록의 `lat`/`lng` 는 가게 위치가 아니라 **그 동네의 중심**입니다.
- 등록할 때 `지금 있는 곳으로 잡기` 를 누르면 현재 좌표를 쓰지만,
  **소수점 둘째 자리까지만** 남깁니다 (약 1km 격자). 어느 건물인지는 저장하지 않아요.
- 내 위치는 **저장하지 않습니다.** 앱을 끄면 사라져요.
- 지도 버튼은 좌표로 핀을 찍지 않고 **가게 이름으로 검색만** 걸어줍니다.
  동네 중심 좌표로 핀을 찍으면 엉뚱한 건물을 가리키게 되니까요.

예전 버전을 쓰던 사람은 `localStorage` 에 좌표 없는 목록이 들어 있어서, 앱이 뜰 때
기본 목록의 좌표만 조용히 채워 넣습니다 (직접 등록한 곳은 건드리지 않아요).
...
품목·현재고·최소수량·단가·거래처를 넣어두면 최소 수량 이하인 것만 모아 발주서
텍스트를 만들어줍니다. 발주 수량은 `최소수량 × 2 − 현재고` 로 잡고, 거래처별로
묶어서 예상 금액까지 계산해요. 복사하거나 바로 공유할 수 있습니다.

처음 들어가면 "기본 품목 10개 넣기"로 시작할 수 있어요.
