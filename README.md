...
손님이 특정 바의 월정액 패스를 사고, 입장할 때 QR을 보여주면 운영자가 찍어
잔수·도장을 관리하는 기능입니다. 가게 단위라 어느 바든 켤 수 있어요.

**1. SQL 넣기** — `supabase/pass.sql` 을 SQL Editor 에서 실행 (bars.sql 이 먼저).

**2. 운영자 지정** — 바 찾기에는 연계된(패스를 운영하는) 가게만 보입니다. 관리자는
바 찾기 위쪽 **연계 가게만 / 전체 목록** 버튼으로 카카오 전체 목록을 열어 가게를 찾고,
가게 페이지의 하우스 패스 칸에서 **운영자 지정**(닉네임을 적거나 비워서 나를). 목록에
없는 가게는 오른쪽 위 + 로 먼저 등록한 뒤 지정하면, 서버에 정보가 남아 모든 손님에게 보여요.

**3. 상품 만들기** — 마이페이지 > **우리 가게 패스 관리** > 상품·설정.
"기본 5종 한 번에 넣기"로 라이트·올데이·트리플·팀·원데이가 들어가고, 도장 목표와
이달의 한정 칵테일을 적은 뒤 **손님에게 패스 받기**를 켭니다.

**4. 손님 흐름** — 가게 페이지에서 상품 선택 → 신청 → 가게에서 결제 → 운영자가
회원 탭에서 **승인**(시작일 지정) → 손님 홈/마이페이지 "내 패스"에 90초마다 바뀌는
QR. 운영자는 **입장 확인** 탭에서 카메라로 찍거나 코드(`12-A3F9B2`)를 입력합니다.
스캔하면 그날 도장이 찍히고, 이달 n번째 방문이면 보상(한정 칵테일)이 생겨요.
"잔 사용 +1"은 하루 잔수·월 상한을 서버가 검사합니다.

**5. 팀 패스** — 팀장 패스가 승인되면 6자리 초대 코드가 생기고, 팀원은 가게
페이지 > "초대 코드로 팀 패스 참여"로 들어옵니다.

**6. 만료 알림 · 자동 정리** — `.github/workflows/pass-cron.yml` 이 매일 아침
`/api/pass-cron` 을 두드려 끝난 패스를 만료 처리하고 3일 전 알림을 보냅니다.
publish-queue 와 같은 `CRON_SECRET` 을 씁니다.

**7. 앱 안 카드 결제 (선택)** — 토스페이먼츠 정기결제. `js/config.js` 의
`TOSS_CLIENT_KEY` 에 클라이언트 키, Vercel 환경변수 `TOSS_SECRET_KEY` 에 비밀키.
둘 다 있으면 상품을 누를 때 "앱에서 카드로 결제"가 같이 뜨고, 결제 즉시 패스가
시작됩니다. 자동 갱신은 손님이 켜고 끄며, 실패하면 D+1·D+3·D+5 재시도 후 만료.
`test_ck_…/test_sk_…` 키면 실제 돈은 안 나갑니다. 카메라 스캔은 HTTPS(운영 주소)에서만 됩니다.

| 환경변수 | 어디에 | 용도 |
|---|---|---|
| `TOSS_SECRET_KEY` | Vercel | 빌링키 발급·결제 (서버 전용) |
| `CRON_SECRET` | Vercel + GitHub Secrets | pass-cron 호출 인증 (이미 있음) |

**지킬 선** — "무제한"이라는 말은 앱이 막습니다(잔수·상한으로만). 미리 금액을
충전하는 구조가 아니라 기간제라 선불전자지급수단 규제에 걸리지 않게 설계했어요.
