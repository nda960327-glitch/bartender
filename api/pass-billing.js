 ...
 *    앱 → action=confirm    : 1회 결제 승인 (카드·카카오페이·네이버페이·토스페이·삼성페이) + 패스 활성화
 *
...
async function issue(me, body) {
  ...
  const pass = await activatePass(me, plan, st, body, { autoRenew: !!body.autoRenew });
  ...
  await notifyOwners(me, plan, st, "카드");

  return { ok: true, pass, receiptUrl: pay.receiptUrl || null, cardLabel: k.cardLabel };
}

/* 패스 활성화 (카드 자동결제·1회 결제 공용). 신청 중이던 게 있으면 그걸 활성화, 없으면 새로 만듭니다.
...
async function notifyOwners(me, plan, st, how) {
  ...
  await Promise.all(owners.map((o) => sendTo(o.user_id, { title: st.bar_name || "하우스 패스", body: `🎫 ${(prof && prof.nick) || "손님"}님이 ${plan.name}을 ${how}로 결제했어요 (${plan.price.toLocaleString("ko-KR")}원).`, tag: "pass-paid" }).catch(() => 0)));
}

/* 1회 결제 승인 (카드·카카오페이·네이버페이·토스페이·삼성페이)
   앱이 토스 결제창에서 돌아오며 받은 paymentKey/orderId/amount 를 서버가 토스에 최종 승인 요청합니다.
   금액은 앱이 보낸 값이 아니라 상품표 가격으로 검증해요. */
async function confirm(me, body) {
  const { paymentKey, orderId, planId } = body;
  if (!paymentKey || !orderId || !planId) return { error: "결제 정보가 빠졌어요." };
  const plan = (await db("pass_plans?id=eq." + q(planId) + "&active=is.true&select=*"))[0];
  if (!plan) return { error: "지금 판매 중인 상품이 아니에요." };
  const st = (await db("bar_pass_settings?bar_key=eq." + q(plan.bar_key) + "&select=*"))[0];
  if (!st || !st.enabled) return { error: "이 가게는 아직 패스를 받지 않아요." };
  ...
  if (done && done.pass_id) {
    const p = (await db("passes?id=eq." + done.pass_id + "&select=*"))[0];
    if (p) return { ok: true, pass: p, already: true };
  }
  const dup = await db("passes?user_id=eq." + me.id + "&bar_key=eq." + q(plan.bar_key) + "&status=in.(active,grace)&select=id");
