...
self.addEventListener("push", (e) => {
  ...
  e.waitUntil(
    self.registration.showNotification(d.title || "바텐톡", {
      ...
      renotify: true,
      data: { cid: d.cid || null, postId: d.postId || null, meetId: d.meetId || null, admin: !!d.admin, offer: d.offer || null },
    })
  );
});
...
self.addEventListener("notificationclick", (e) => {
  ...
  e.waitUntil((async () => {
    ...
    for (const w of wins) {
      ...
      await w.focus();
      if (d.offer) w.postMessage({ type: "open-offer", offer: d.offer });
      else if (d.postId) w.postMessage({ type: "open-post", postId: d.postId });
    ...
    const url = d.offer ? "./?offer=" + encodeURIComponent(d.offer)
      : d.postId ? "./?post=" + d.postId
