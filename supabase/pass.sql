...
declare
  ...
  members int; pending int; visits int; sides int; tt_days int; tt_visits int; drinks int; revenue int;
begin
  if not (public.is_bar_owner(p_bar) or public.is_admin()) then raise exception '이 가게 운영자만 볼 수 있어요.'; end if;
  select count(*) into members from public.passes where bar_key = p_bar and status in ('active', 'grace');
  select count(*) into pending from public.passes where bar_key = p_bar and status = 'requested';
  select count(*), count(*) filter (where side) into visits, sides
    from public.pass_visits where bar_key = p_bar and action = 'enter' and day >= m0;
  select count(distinct day), count(*) into tt_days, tt_visits
    from public.pass_visits where bar_key = p_bar and action = 'enter' and day >= m0 and extract(isodow from day) between 2 and 4;
  select coalesce(sum(drinks), 0) into drinks from public.pass_visits where bar_key = p_bar and day >= m0;
  select coalesce(sum(price), 0) into revenue from public.passes
    where bar_key = p_bar and approved_at >= m0 and status in ('active', 'grace', 'expired');
  return json_build_object(
    'members', members, 'pending', pending,
    'visits_month', visits,
    'avg_visits', case when members > 0 then round(visits::numeric / members, 1) else 0 end,
    'nightly_tt', case when tt_days > 0 then round(tt_visits::numeric / tt_days, 1) else 0 end,
    'side_rate', case when visits > 0 then round(100.0 * sides / visits) else null end,
    'drinks_month', drinks, 'revenue_month', revenue,
